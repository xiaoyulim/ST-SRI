"""
实验三（E3）：生理验证 — EMD 延迟检测
======================================
目标：在真实 sEMG 数据上验证 ST-SRI 检测到的协同峰是否落入
      已知的电机械延迟（EMD）范围（30-100ms）。

方法：
  - 对每个受试者的 per-subject LSTM 模型运行 ST-SRI 扫描
  - 检测平均协同谱的峰值位置
  - 统计峰值落入 30-100ms 范围的比例

评价指标：
  - 峰值均值与中位数
  - 有效峰值（落入 EMD 范围）的受试者比例
  - 跨受试者峰值分布
"""
import sys, os
